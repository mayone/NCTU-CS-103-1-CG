 ...
 * * Filename: 		main.cpp
 * * Description: 	display the TestScene2
 * * 
 * * Version: 		1.0
 * * Created:		2015/1/1
 * * Revision:		2015/1/14
...
{
	// load data from file
	light_data = new light("TestScene2.light", 1);
	view_data = new view("TestScene2.view");
	scene_data = new scene("TestScene2.scene", 2);
...
void drawObjects()
{
	...
	for(int objID = 0; objID < scene_data->obj_num; objID++)
	{
		...
		SaveTRS_start(	scene_data->T[objID][0], scene_data->T[objID][1], scene_data->T[objID][2],
						scene_data->Angle[objID], scene_data->R[objID][0], scene_data->R[objID][1], scene_data->R[2][2],
